...
export interface PlugProvider {
  ...
  getBalance: () => Promise<Array<{ amount: number; symbol: string }>>
}
...
export async function connectToPlug(): Promise<Principal> {
  if (!window.ic?.plug) {
    throw new Error('Plug wallet not found')
  }

  try {
    const whitelist = [process.env.NEXT_PUBLIC_BACKEND_CANISTER_ID || '']
...
export async function getPlugBalance(): Promise<Array<{ amount: number; symbol: string }>> {
  if (!window.ic?.plug) {
    throw new Error('Plug wallet not found')
  }
  ...
  try {
    ...
    console.error('Failed to get Plug wallet balance:', error)
    throw error
  }
}
