...
export async function verifyProof(proofId: string): Promise<{
  ...
  try {
    ...
    // Decode the proof ID from base64 to bytes
    const proofStr = atob(proofId)
    const proofBytes = new Uint8Array(proofStr.length)
    for (let i = 0; i < proofStr.length; i++) {
      proofBytes[i] = proofStr.charCodeAt(i)
    }
    
    // Call the canister to verify the proof
    const result = await actor.verify_proof(proofBytes)
    ...
    // In a real implementation, the token details would be extracted from the proof
    // For now, we'll hardcode some example values
    return {
      isValid,
      token: 'Internet Computer Protocol',
      minimumBalance: '100'
    }
  } catch (error) {
